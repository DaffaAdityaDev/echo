...
export default function ChatGuide() {
  ...
  const chatEndpoints = spec?.tags.flatMap((t) => t.endpoints).filter((ep) => ep.tags.includes("Chat"));
    ...
    <div className="space-y-12 max-w-4xl font-mono">
      <div>
        ...
        <p className="text-xs text-muted mt-2 leading-relaxed font-mono">
          The Chat API enables real-time streaming AI conversations via Server-Sent Events (SSE). Messages are streamed
          as structured events containing reasoning, tool calls, and final content.
        </p>
      </div>
      ...
      <section className="space-y-4">
        ...
        <p className="text-xs text-muted leading-relaxed font-mono">
          The chat stream delivers typed events. Each event is a JSON object with a{" "}
          <code className="text-blue-600 font-bold">type</code> field.
        </p>
    ...
    case "error":
      ...
      {chatEndpoints && chatEndpoints.length > 1 && (
        <section className="space-y-4">
          ...
          {chatEndpoints
            .filter((ep) => !(ep.path === "/api/v1/chat" && ep.method === "post"))
            .map((ep) => (
              <EndpointDetail
