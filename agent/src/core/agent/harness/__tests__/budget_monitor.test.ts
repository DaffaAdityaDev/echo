...
describe('BudgetMonitor', () => {
  ...
  it('within budget returns exceeded=false', () => {
    ...
    expect(m.checkBudget(5, 0.50)).toEqual({ exceeded: false });
  });
  ...
  it('exceeding cost cap returns COST_CAP_EXCEEDED', () => {
    const m = new BudgetMonitor({ enabled: true, maxCostUsd: 1.00, enforceCostCap: true });
    const r = m.checkBudget(1, 1.50);
    expect(r.exceeded).toBe(true);
  ...
  it('steps are checked before timeout and cost', () => {
    ...
    const m = new BudgetMonitor({ enabled: true, maxSteps: 5, maxDurationMs: 1, maxCostUsd: 0.01, enforceMaxSteps: true, enforceTimeout: true, enforceCostCap: true });
