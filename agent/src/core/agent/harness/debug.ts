...
export function queuePromptDebug({ state, iteration, strategyName, systemPrompt }: DebugLedgerOptions): void {
  setImmediate(async () => {
    try {
      ...
      const messageHistoryString = state.messages
        .map((m, idx) => {
      ...
      const mdContent = [
        HARNESS_PROMPTS.MD_LEDGER_HEADER(
          timeString,
          shortMissionId,
          iteration,
          strategyName,
      ...
      const pureMessagesString = state.messages
        .map((m, idx) => {
      ...
      const pureContent = HARNESS_PROMPTS.PURE_LEDGER(
        iteration,
        shortMissionId,
        timeString,
        systemPrompt,
