...
Format your response immediately as the final conclusion for the user.`,
  ...
  REPEATING_WARNING: `SYSTEM SYSTEM WARNING: You are repeating your previous reasoning path. Your last action failed with a silent state conflict. Do not attempt the same tool configuration again. Choose a different diagnostic endpoint or execute a graceful exit.`,

  RECOVERY_PROMPT: `[HARNESS SYSTEM ERROR]: You attempted to call a tool using raw XML tags but the parameters were malformed. Please use the strict native tool call protocol or fix your XML block format immediately. Do not chat, execute the tool.`,
  ...
  LOG_RE_ROUTE: `[Harness Notice]: Tool execution automatically re-routed via Soft Recovery.`,
