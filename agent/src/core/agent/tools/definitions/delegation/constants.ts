export const DELEGATION_CONFIG = {
  NAME: "delegate_task",
  DESCRIPTION:
    "Delegate a specific sub-task or research query to a specialized child/sub-agent. This isolates the parent workspace and keeps the context window clean.",
  KEYWORDS: [
    "delegate",
    "sub-agent",
    "subagent",
...
export const DELEGATION_DEFAULTS = {
  ...
  SUMMARY_FAILURE: (agentName: string, errorMsg: string) =>
    `Sub-agent [${agentName}] failed to execute task: ${errorMsg}`,
} as const;
