...
export const read_file: ToolDefinition = {
    ...
    execute: async (input: { filename: string; type: 'artifact' | 'runtime' }): Promise<Observation> => {
        try {
            ...
            let summary = `Failed to read file: ${error.message}`;
            if (error.code === 'ENOENT') {
                summary = `Error: File '${input.filename}' was not found in the '${input.type}' workspace directory. You must create it first using the 'write_file' tool before attempting to read it.`;
            }
            return {
                status: 'error',
                summary,
