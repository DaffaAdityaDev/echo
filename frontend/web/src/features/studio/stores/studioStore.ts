...
import type { StreamPacket, AgentStatus, TokenUsage, MissionMeta } from "@/features/chat/types"
...
export const useStudioStore = create<StudioState>((set) => ({
  ...
  appendDebugPacket: (packet) => set((state) => {
  ...
  appendDebugInfo: (info) => set((state) => {
  ...
  addDebugContent: (chunk) => set((state) => ({
    debugContent: state.debugContent + chunk,
  })),

  addDebugReasoning: (chunk) => set((state) => ({
    debugReasoning: state.debugReasoning + chunk,
  })),

  addDebugSubagentCall: (name, instruction) => set((state) => {
  ...
  addDebugSubagentResult: (name, instruction, result, status) => set((state) => {
