...
export const useDebugStore = create<DebugState>((set) => ({
  ...
  appendPacketLog: (packet) => set((state) => {
  ...
  addStreamingContent: (chunk) => set((state) => ({
    streamingContent: state.streamingContent + chunk,
  })),

  addReasoningChunk: (chunk) => set((state) => ({
    streamingReasoning: state.streamingReasoning + chunk,
  })),

  addSubagentCall: (name, instruction) => set((state) => {
  ...
  addSubagentResult: (name, instruction, result, status) => set((state) => {
  ...
  addToolCall: (name, args, timestamp) => set((state) => {
  ...
  addToolResult: (name, content, timestamp) => set((state) => {
  ...
  setDegradation: (from, to, reason) => set((state) => {
  ...
  setMissionMeta: (meta) => set({
    missionMeta: meta,
    maxIterations: meta.maxIterations ?? 15,
  }),

  addStateChange: (from, to, reason, timestamp) => set((state) => ({
