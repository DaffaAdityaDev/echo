...
export function ModelComparisonGrid({ results, isLoading, streamingContent, streamingReasoning, selectedModels }: ModelComparisonGridProps) {
  if (!isLoading && (!results || results.length === 0)) return null

  if (isLoading && (!streamingContent || Object.keys(streamingContent).length === 0) && (!streamingReasoning || Object.keys(streamingReasoning).length === 0) && (!results || results.length === 0)) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(selectedModels ?? [1, 2, 3]).map((m, i) => (
          <div key={typeof m === "string" ? m : i} className="border border-zinc-200 bg-zinc-50 rounded-2xl p-4 space-y-3">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {(() => {
        ...
        if (result) {
          return (
            ...
            >
              ...
              {result.error ? (
                <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg p-2">{result.error}</div>
              ) : (
        ...
        return (
          <div
            key={model}
            className="border border-blue-200 bg-blue-50/50 rounded-2xl p-4 space-y-3 transition-all"
          >
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-zinc-800">{model}</h3>
