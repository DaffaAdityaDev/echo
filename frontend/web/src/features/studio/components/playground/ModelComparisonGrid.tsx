  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="border border-zinc-200 bg-zinc-50 rounded-2xl p-4 space-y-3">
      ...
      {results.map((r, idx) => (
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-zinc-800">{r.model}</h3>
          ...
          {r.error ? (
            <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg p-2">{r.error}</div>
          ) : (
