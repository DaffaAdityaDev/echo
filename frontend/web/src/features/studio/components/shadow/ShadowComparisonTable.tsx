        ...
        return (
          <div key={run.id}>
            <button
              ...
              className="w-full flex items-center gap-4 px-4 py-3.5 hover:bg-zinc-100/50 transition-colors text-left"
            >
              <div className="shrink-0">
                {isExpanded ? <ChevronDown className="h-4 w-4 text-zinc-500" /> : <ChevronRight className="h-4 w-4 text-zinc-500" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm text-zinc-800 font-medium truncate">{run.user_query}</div>
              </div>
              <div className="flex items-center gap-3 text-xs shrink-0">
                <span className="text-zinc-500">{new Date(run.created_at).toLocaleDateString()}</span>
                <span className={cn("flex items-center gap-1 font-medium", liveBetter ? "text-emerald-600" : "text-zinc-500")}>
                  {liveBetter ? <Check className="h-3 w-3" /> : null} Live {run.live_cost_usd.toFixed(4)}
                </span>
                <span className={cn("flex items-center gap-1 font-medium", shadowBetter ? "text-blue-600" : "text-zinc-500")}>
              ...
              <div className="px-4 pb-4 space-y-2">
                <div className="grid grid-cols-2 gap-4">
                  <div className="border border-zinc-200 bg-white rounded-xl p-3">
                    ...
                    <pre className="text-xs text-zinc-800 font-mono whitespace-pre-wrap max-h-32 overflow-y-auto">{run.live_output}</pre>
                  ...
                  <div className="border border-blue-100 bg-blue-50/20 rounded-xl p-3">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-blue-600 mb-1">Candidate (v{run.candidate_version_id.slice(0, 8)})</div>
                    <pre className="text-xs text-zinc-800 font-mono whitespace-pre-wrap max-h-32 overflow-y-auto">{run.shadow_output}</pre>
                    <div className="flex gap-3 mt-2 text-[10px] text-zinc-500">
