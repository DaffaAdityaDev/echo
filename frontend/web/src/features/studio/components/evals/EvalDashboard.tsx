...
export function EvalDashboard({ evalRun, isRunning, error, onRun }: EvalDashboardProps) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-zinc-900">Eval Suite</h1>
        <p className="text-sm text-zinc-500 mt-1">Automatically grade your prompts against test datasets.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="border border-zinc-200 bg-zinc-50/50 rounded-2xl p-5 space-y-4">
            ...
            {testCases.length > 0 && (
              <div className="text-xs text-zinc-500">
                Loaded <span className="text-zinc-900 font-semibold">{testCases.length}</span> test cases
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          {error && (
            <div className="flex items-center gap-2 text-xs text-red-600 bg-red-50 rounded-xl px-4 py-3 border border-red-200">
              <AlertCircle className="h-4 w-4 shrink-0" />
              {error}
            </div>
          )}
          {isRunning && (
            <div className="border border-zinc-300 bg-zinc-50/50 rounded-2xl p-5 text-center space-y-3">
              <Clock className="h-6 w-6 text-blue-600 mx-auto animate-spin" />
          ...
          {!evalRun && !isRunning && !error && (
            <div className="flex flex-col items-center justify-center h-64 border border-zinc-300 bg-zinc-50/50 rounded-2xl text-center space-y-2">
              <ClipboardCheck className="h-8 w-8 text-zinc-500" />
