...
export default function AuthGuide() {
    ...
    <div className="space-y-12 max-w-4xl font-mono">
      <div>
        ...
        <p className="text-xs text-muted mt-2 leading-relaxed font-mono">
          Echo API supports two authentication methods: JWT bearer tokens for user-facing applications
          and long-lived API keys for server-to-server integration.
        </p>
      </div>
  ...
  -d '{"email": "user@example.com", "password": "your-password"}'`}
        ...
        <p className="text-xs text-muted">
          Response includes a <code className="text-blue-600 font-bold">token</code> field. Include this in all subsequent requests:
        </p>
        <CodeBlock
          language="http"
      ...
      <section className="space-y-4">
        ...
        <p className="text-xs text-muted leading-relaxed font-mono">
          For automated or server-side integrations, create an API key via the admin console
          or admin API endpoint. API keys are long-lived and scoped to specific permissions.
        </p>
        <div className="p-4 border border-border bg-slate-50/80 rounded-xs flex gap-3.5 shadow-xs">
          <Info className="h-5 w-5 text-amber-600 shrink-0 mt-0.5" />
          <p className="text-xs text-muted">
            API key management requires admin privileges. Use the Admin Console at <code className="text-blue-600 font-bold">/admin</code> or the Admin API endpoints.
          </p>
        </div>
      </section>
...
Authorization: Bearer <your-api-key>`}
        ...
        <p className="text-xs text-muted font-mono">
          Unauthenticated requests to protected endpoints return a <span className="text-rose-600 font-bold">401 Unauthorized</span> status.
        </p>
      </section>
    </div>
  )
