...
| 8 | Complexity inflation | Long functions; deep nesting; oversized parameter lists; dumping-ground files | Small functions, early returns, split handlers/components. Code review MUST flag functions > ~50 lines or > 3 nesting levels. **File-size rule: any file > 500 lines MUST be reviewed for splitting into cohesive modules; > 800 lines MUST be split unless it is a single-purpose module that is inherently one unit (e.g., a generated file, a schema/contract, or one cohesive class with no separable concern). Sub-folders are preferred over flat dumping grounds (`components/debug/`, `services/stream/handlers/`, `harness/*.ts`)** |
