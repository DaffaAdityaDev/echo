...
| Delegate filter          | `harness/harness.ts:135` (selectTools) | Sub-agents filter out delegate_task                   |
| Tool map                 | `harness/harness.ts:135` (selectTools) | `Map<string, ToolDefinition>` for O(1) lookup         |
